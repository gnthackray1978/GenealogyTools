...
using ChildRelationship = FTMContextNet.Domain.Entities.Source.ChildRelationship;
using Person = FTMContextNet.Domain.Entities.Persistent.Source.Gedcom.Person;


namespace GedcomParser.Services
{
    /// <summary>
    /// For reading in ged files.
    /// </summary>
    public class GedPseudoContext
    {
        ...
        public static GedPseudoContext Create(string filePath)
        {
            var fileParser = new GedPseudoContext();
        ...
        {
            var person = new Person { StrId = indiChunk.Id };
        ...
        private void ParseFamily(GedcomChunk famChunk)
        {
            ...
            foreach (var chunk in famChunk.SubChunks)
            {
                ...
                {
                    case "CHIL":
                        var child = PersonContainer.Persons.SingleOrDefault(p => p.StrId == chunk.Reference);
                        if (child != null)
                        {
                            children.Add(child);
                        }
                    ...
                    case "HUSB":
                        var husband = PersonContainer.Persons.SingleOrDefault(p => p.StrId == chunk.Reference);
                        if (husband != null)
                        {
                            parents.Add(husband);
                        }
                    ...
                    case "WIFE":
                        var wife = PersonContainer.Persons.SingleOrDefault(p => p.StrId == chunk.Reference);
                        if (wife != null)
                        {
                            parents.Add(wife);
                        }
            ...
            {
                foreach (var child2 in children.Where(c => c.StrId != child1.StrId))
                {
        ...
        private DatePlace PopulatePlaceData(GedcomLine line, DatePlace datePlace)
        {
            if(datePlace == null)
                datePlace = new DatePlace();

            if (line.Type == "DATE") datePlace.Date = line.Data;
            if (line.Type == "PLAC") datePlace.Place = line.Data;

            if (line.Type == "NOTE") datePlace.Note = line.Data;
        ...
        {
            var childChunk = _idChunks.SingleOrDefault(c => c.Id == childRelation.From.StrId);
            if (childChunk != null)
            {
