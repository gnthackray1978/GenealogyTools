...
namespace GenDBContextNET.Contexts
{
    ...
    // some people have described ef contexts as repos in their own right.
    // im using it as a tool for querying the db
    // i dont agree see posts on here
    // https://softwareengineering.stackexchange.com/questions/387135/should-entity-framework-6-not-be-used-with-repository-pattern
    ...
    public partial class DNAGEDContext : DbContext
    {
        ...
        {
            var oldPersons = Persons.Where(pid => pid.RootsEntry);

            Console.WriteLine("Removing  " + oldPersons.Count() + " old records");

            RemoveRange(oldPersons);
        ...
        {
            Database.ExecuteSqlRaw("DELETE FROM PersonsOfInterest");
        ...
        {
            var s = Database.GetDbConnection().ConnectionString;
