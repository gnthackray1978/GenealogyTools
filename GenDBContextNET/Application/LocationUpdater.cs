...
using GenDBContextNET.Data.Repositories;
using LoggingLib;
using PlaceLib;
using PlaceLibNet;
using PlaceLibNet.Data.Contexts;
using PlaceLibNet.Data.PlaceMappings;
using PlaceLibNet.Data.Repositories;

namespace GenDBContextNET.Application
{
    public class LocationFixer
    {
        ...
        public void UpdateBritishParents()
        {
            ...
            int total = personsBornInEngland.Count();
            int counter = 0;
        ...
        public void UpdateCountry(bool countryUpdated)
        {
            ...
            foreach (var p in places)
            {
                if(!countryDictionary.ContainsKey(p.personId))
                    countryDictionary.Add(p.personId, PlaceMapCountry.Find(p.place));
        ...
        public static void UpdateLocations()
        {
            ...
            var iMSGConfigHelper = new MSGConfigHelper();

            var locationFixer = new LocationFixer(new LocationSqlLiteDataContext(), consoleWrapper);
            var placeRepo = new PlacesRepository(new PlacesContext(iMSGConfigHelper), consoleWrapper);
            ...
            locationFixer.SetBirthCounty(placeRepo.GetCounties());
